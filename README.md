...
Claude Code ──(JSON-RPC/stdio)──> Arrow MCP Server
                                    ...
                                    ├── Context Assembler (auto token budget)
                                    ├── Impact Analyzer (callers, tests, deps)
                                    ├── Cross-Repo Resolver (symbol lookup)
                                    ├── Diff Context Engine (git + AST)
                                    ├── Test Mapper (import + naming conventions)
                                    ├── Structure Analyzer (symbols + imports)
                                    ├── File Watchers (one per project, watchdog)
                                    ├── Auto-Warm (background index on startup)
